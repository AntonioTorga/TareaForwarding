...
def create_packet(parsed_packet):
    return (parsed_packet["IP"]+","+str(parsed_packet["PORT"])+","+str(parsed_packet["TTL"])+","+parsed_packet["MSG"]).encode()
...
while True:
    ...
    if msg["IP"]==router_ip and msg["PORT"]==router_port:
        ...
        if tupla == None:
            ...
            continue
        ip, port = tupla
        og = create_packet({"IP":msg["IP"],"PORT":msg["PORT"],"TTL":msg["TTL"]-1,"MSG":msg["MSG"]})
